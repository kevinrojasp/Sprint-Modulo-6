...
<h2 class="text-center display-6" style="margin-top: 20px;">Listado de capacitaciones</h2>
    ...
    <div class="row">
        <div class="table-responsive">
            <table class="table" id="example">
                ...
                <tbody>
                <c:forEach var="capacitacion" items="${listadoCapacitaciones}">
                    <tr>
                        <td>${capacitacion.id}</td>
                        <td>${capacitacion.nombre}</td>
                        <td>${capacitacion.fecha}</td>
                        <td>${capacitacion.hora_inicio}</td>
                        <td>${capacitacion.hora_termino}</td>
                        <td>${capacitacion.lugar}</td>
                        <td>${capacitacion.nro_asistentes}</td>
                        <td>${capacitacion.detalle}</td>
                        <td>
                            <form action="/editar-capacitacion" method="get">
                                <input type="hidden" name="id" id="id" value="${capacitacion.id}" />
                        ...
                        <td>
                            <form action="/eliminar-capacitacion" method="get">
